# Deploying a nginx `Pod` with a `Service` of type `NodePort`

    $ kubectl apply -f deployment-nginx-http-nodeport.yaml

# Deploying a nginx `Pod` with a `Service` of type `LoadBalancer`

    $ kubectl apply -f deployment-nginx-http-load_balancer.yaml

# Deploying a nginx `Service` with HTTPS enabled
...
### Mounting the TLS `Secret` created into the curlpod
